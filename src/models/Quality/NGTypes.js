...
function typeCodeValidate (rule, typeCode, callback) {
  ...
  if (!typeCode) {
    return callback(new Error('不良类型编号不能为空'))
  }
  Api.get('NGTypes/Validate', { typeCode }).then(valid => {
    if (valid) {
      callback()
    } else {
      callback(new Error('不良类型编号已存在'))
    }
  })
}
...
export default {
  ...
  // 表格列配置
  cols: [
    {field: 'typeCode', title: '不良类型编号', width: 250, titleAlign: 'left', columnAlign: 'left', isResize: true, isEdit: true},
    {field: 'typeName', title: '不良类型名称', width: 250, titleAlign: 'left', columnAlign: 'left', isResize: true},
  ...
  getForm (form = null, type = 'add', opts) {
    return Promise.resolve({
      ...
      rules: {
        ...
        typeName: [{ required: true, message: '请输入不良类型名称', trigger: 'blur' }]
      }
    })
  }
}
